...
<title>Insert title here</title>
	...
	ArrayList<MyDeptDTO> deptlist = 
				(ArrayList<MyDeptDTO>)request.getAttribute("deptlist");
	%>
	...
	<table border="1" width="600">
		...
		for(int i=0;i<size;i++){
			...
			<tr>
				...
				<td><a href="/template/deptread.do?deptno=<%=deptInfo.getDeptno()%>&state=READ"><%= deptInfo.getDeptname() %></a></td>
				<td><%= deptInfo.getLoc() %></td>
				<td><%= deptInfo.getTelNum() %></td>
				<td>
				<a 
		href="/template/deptdelete.do?deptno=<%=deptInfo.getDeptno() %>">삭제</a></td>
			</tr>			
		<%}%>
